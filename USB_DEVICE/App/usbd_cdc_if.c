...
{
  /* USER CODE BEGIN 6 */
  parse_packet(Buf, *Len, &g_usb_message);
  //PRINTF_CUSTOM_USB_MSG(g_usb_message);
    
  if (g_usb_message.valid)
  {
      cmd_function func = g_cmd_table[g_usb_message.cmd].func_handler;
      
      if (func != NULL)
      {
          msg_data tmp_payload = {0};
          uint8_t ack_flag = func(&(g_usb_message.payload), &tmp_payload);
