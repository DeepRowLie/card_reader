...
uint8_t Read_MFRC522(uint8_t addr) {
  //uint8_t32_t rx_bits;
  uint8_t rx_bits = 0x80; // spi tx dummy data:read 0x00
...
//  MSS_GPIO_set_output( MSS_GPIO_1, 1 );
  ...
  // Timer: (2 * TPrescaler + 1)*(TreloadVal + 1) / 13.56MHz = 24ms
  Write_MFRC522(TModeReg, 0x80);      // 0x8D);      // Tauto=1;
